...
<img width="1672" height="940" alt="Banner" src="https://github.com/user-attachments/assets/5ef22f81-cf0f-4ff8-a1ec-3f856981b5ba" />
...
🎬 **Video de presentación:** https://youtu.be/PnOKb8TFxds
...
- [Implementado recientemente](#-implementado-recientemente)
- [Roadmap](#-roadmap)
...
Cada vector almacenado en Pinecone tiene asociada metadata que permite citar la fuente en la respuesta. Hoy, cada registro guarda solo dos campos:

```json
{
  "source": "18.2. TRATADO DE TLATELOLCO.pdf",
...
**Limitación conocida: no hay número de página.** El esquema de metadata inicialmente planeado para este proyecto incluía campos como `titulo`, `seccion`, `pagina` y `fecha_vigencia`, pero el índice real solo terminó guardando `source` (el nombre del archivo) y `text`. Esto se confirmó revisando un registro directamente en el dashboard de Pinecone. Como consecuencia, las citas que genera el asistente indican la norma o ley usada, pero **nunca un número de página** — el prompt del sistema le indica explícitamente al modelo que no lo incluya, justamente para no inventar un dato que la fuente real no tiene. Si en algún momento se quiere citar con página, hay que volver a construir el pipeline de indexación para que extraiga y guarde esa metadata por cada chunk — no es algo que se resuelva solo con cambios en el prompt.
...
- **Botón de copiar respuesta** al portapapeles, en cada mensaje del asistente.
- **Exportar conversación** completa como archivo `.txt`, desde la sidebar.
- **Contador de caracteres** en el input, con aviso visual al acercarse al límite.
- **Soporte de tablas markdown** en las respuestas (además de títulos y separadores).
- **Meta tags Open Graph / Twitter Card**, para que el link de la demo muestre una vista previa con imagen al compartirse (WhatsApp, Slack, etc.).
...
| Media | 📎 Agregar `pagina` al pipeline de indexación de Pinecone | Confirmado (agosto 2026) que el índice actual no la tiene — hoy las citas nunca muestran página a propósito, para no inventar el dato |
